...
def panel_plot(DATA,N,**kwargs):
    ...
    for ax1,(likelihoods,info) in zip(AX,DATA):
        ...
        for (a,b) in pairs: #default_classification[key]:
...
def detect_haploids_and_triploids_old(cases):
    ERRORS = []
    HAPLOIDS = []
    TRIPLOIDS = []
...
def detect_haploids_and_triploids(cases):
    ...
    for case in cases:
        ...
        if len(LLRs_aggregation[pairs[0]])<10: continue
        ...
        if M[('DISOMY','BPH')]/SD[('DISOMY','BPH')]<1 and len(LLRs_aggregation[pairs[0]])>=1000 and sum(DEPTHS)/len(DEPTHS)>0.006:
            A = {'mean': M[('DISOMY','BPH')], 'SD': SD[('DISOMY','BPH')] }
            print('Triploid',bam_filename.replace('.bam',''), M[('DISOMY','BPH')], SD[('DISOMY','BPH')], case['sp'], len(LLRs_aggregation[pairs[0]]),case['sex'])
            case.update(A)
            TRIPLOIDS.append(case)
        elif M[('MONOSOMY','DISOMY')]/SD[('MONOSOMY','DISOMY')]>1 and len(LLRs_aggregation[pairs[0]])>=250 and sum(DEPTHS)/len(DEPTHS)>0.004:
            A = {'mean': M[('MONOSOMY','DISOMY')], 'SD': SD[('MONOSOMY','DISOMY')] }
            print('Haploid',bam_filename.replace('.bam',''), M[('MONOSOMY','DISOMY')], SD[('MONOSOMY','DISOMY')], case['sp'], len(LLRs_aggregation[pairs[0]]),case['sex'])
            case.update(A)
            HAPLOIDS.append(case)
...
def count(cases):
    ...
    for case in cases:
        ...
        if len(LLRs)<22:
            ...
            if len(LLRs_aggregation[pairs[0]]) < 250 or sum(DEPTHS)/len(DEPTHS)<0.004:
                counter_haploids += 1    
                
            if len(LLRs_aggregation[pairs[1]]) < 1000 or sum(DEPTHS)/len(DEPTHS)<0.007:
                counter_triploids += 1
                        
    return counter_haploids, counter_triploids
...
if __name__ == "__main__":
    ...
    #DATA = [load_likelihoods(f'/home/ariad/Dropbox/postdoc_JHU/LD-PGTA_ecosystem/LD-PGTA_V2/results_ZOUVES/{identifier:s}.chr{i:d}.LLR.p.bz2') for i in range(1,23)]
    #panel_plot(DATA,N=10,title=f'{identifier:s}')
    
    #likelihoods, info = load_likelihoods('/home/ariad/Dropbox/postdoc_JHU/LD-PGTA_ecosystem/LD-PGTA_V2/results_ZOUVES/12751FA-AWL31_14.chr7.LLR.p.bz2')
    ...
    with open(HOME+'/ariad/Dropbox/postdoc_JHU/Zouves-BlueFuse/Play/clean_diploids.p', 'rb') as f:
    ...
    #counter_haploids, counter_triploids = count(db_TEST)
