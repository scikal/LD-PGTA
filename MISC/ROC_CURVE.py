...
import pickle, statistics, bz2, gzip
...
def main(RATIO,SP,matched,mixed,C,buckets_in_chr21):
    ...
    with open(f"data/PREDICTED_RATES_FOR_{RATIO[0]:s}_vs_{RATIO[1]:s}_{matched:s}_{SP:s}_{conf['depth']:g}x_{buckets_in_chr21:d}BUCKETS.p" , "wb") as f:
        pickle.dump(R, f, protocol=4) #BTPR vs. BFPR for each bin along the genome
        pickle.dump(configuration(C), f, protocol=4) #Simulation info
        pickle.dump(N, f, protocol=4) #Statistical info
        pickle.dump(T, f, protocol=4) #Bin average


if __name__ == "__main__":
    #RATIO = ('BPH','disomy')
    proc = []
    for RATIO in (('BPH','SPH'),('BPH','disomy'),('monosomy','disomy'),('SPH','disomy')):
        for SP in  ('EUR','EAS','SAS','AMR','AFR'): #('AFR_EUR','EAS_SAS','SAS_EUR', 'EAS_EUR'): #
            for matched,mixed in (('MISMATCHED','_mixed'),('MATCHED','')):
                for C in ('C0','C2','C1'):
                    for buckets_in_chr21 in (5,10):
                        ...
                        #buckets_in_chr21 = 5
                        try:
                            p = Process(target=main,args=(RATIO,SP,matched,mixed,C,buckets_in_chr21))
                            p.start()
                            proc.append(p)
                        except:
