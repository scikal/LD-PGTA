...
def unit_test():
    ...
    are_equivalent = lambda x,y: all(abs(i-j)<1e-10 for i,j in zip(x,y)) and len(x)==len(y)
    ...
    for i in range(1000):
        ...
        assert are_equivalent(frequencies0(alleles[x+0:x+1]).values(), frequencies_redundant0(alleles[x+0:x+1]).values())
        assert are_equivalent(frequencies0(alleles[x:x+4]).values(), frequencies_redundant0(alleles[x:x+4]).values())
        assert are_equivalent(frequencies1(alleles[x+0:x+1]).values(), frequencies_redundant1(alleles[x+0:x+1]).values())
        assert are_equivalent(frequencies1(alleles[x:x+4]).values(), frequencies_redundant1(alleles[x:x+4]).values())
        assert are_equivalent(frequencies0(haplotypes).values(), frequencies_redundant0(haplotypes).values())
        assert are_equivalent(frequencies1(haplotypes).values(), frequencies_redundant1(haplotypes).values())
        assert are_equivalent(A.likelihoods(haplotypes), A.likelihoods4(haplotypes))
        assert are_equivalent(frequencies0(alleles[x:x+2]).values(), frequencies_redundant0(alleles[x:x+2]).values())
        assert are_equivalent(frequencies1(alleles[x:x+2]).values(), frequencies_redundant1(alleles[x:x+2]).values())
        assert are_equivalent(A.likelihoods(alleles[x:x+2]), A.likelihoods2(alleles[x:x+2]))
        assert are_equivalent(frequencies0(alleles[x:x+3]).values(), frequencies_redundant0(alleles[x:x+3]).values())
        assert are_equivalent(frequencies1(alleles[x:x+3]).values(), frequencies_redundant1(alleles[x:x+3]).values())
